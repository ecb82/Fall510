...
explore <- function(dataframe,binvec,cthresh){
  ...
  for(n in 1:len_num){
    ...
    for(i in binvec){
      ...
      countplt <- countplt +  geom_histogram(binwidth = i, colour='blue') + geom_vline(xintercept=varmean,colour='red') +labs(xlab=varname)
      
      #plot the density
      densplt <- ggplot(numvar, aes(x=temp_var),environment = environment())
      densplt <- countplt +  geom_histogram(aes(y= ..density..),binwidth = i, colour='blue') + geom_vline(xintercept=varmean,colour='red')+labs(xlab=varname)
